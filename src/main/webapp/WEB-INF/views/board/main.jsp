...
<body>
	...
	<main>
		...
		<div style="margin: 25px 0; width: 100%; display: flex; justify-content:right; align-items: center;">
		<!-- <a style="width: 100px; height: 30px;">등록</a> -->
		</div>
	...
	$(document).ready(function() {
		  ...
		  function updateTable(data) {
		    ...
		    data.forEach(function(board) {
		      html += '<tr>';
		      html += '<td>' + board.noti_id + '</td>';
		      html += '<td><a href="' + root + '/board/read?noti_id=' + board.noti_id + '">' + board.title + '</a></td>';
		      html += '<td>' + board.a_id + '</td>';
		      html += '<td>' + board.registration + '</td>';
		      html += '<td></td>';
		      html += '</tr>';
		    });
		    $('#list').html(html);
		  ...
		  function loadData(filter, searchText) {
		    $.ajax({
		      url: "${root}api/NotiListFiltering",
		      type: "POST",
		      data: {
		        filter: filter,
